...
array = [ map(int,'08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08'.split(' ')),
...
diag = [matrix[::-1,:].diagonal(i) for i in range(-matrix.shape[0]+1,matrix.shape[1])]
for x in diag:
    if len(x) > 4:
        for y in xrange(0,len(x)):
            window = x[y:y+4]
            if len(window) == 4 and 0 not in window:
                product = reduce(lambda res,x: res*int(x), window, 1)
                if maximum < product:
                    maximum = product
